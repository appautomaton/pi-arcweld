...
Plan mode is a safe, read-only space. If the user is only asking a question or getting oriented, it stays out of the way and never demands a plan. The handoff appears only when the model records an *actionable* plan: an `update_todos` list with at least one step still `pending`, which cannot be carried out while writes are blocked. Pure exploration completes its todos as it reads, so nothing is pending and no menu appears. After the agent is fully settled, a pending plan offers Execute, Stay, or Refine. Execute revalidates the current todo list, lifts the restrictions, and starts the first step through a visibly labeled `plan-mode-execute` custom message. It never queues an instruction that appears as if the user typed it. The `pi-arcweld-todos` widget tracks progress from there. Plan mode reads only the tool name `update_todos` and its `details.todos`, with no code dependency on that extension.
